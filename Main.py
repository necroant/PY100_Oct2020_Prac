...
# Конструкция `list(range(start, end, step]))` позволяет получить последовательность (список)
# целых чисел, начинающуюся со `start`, заканчивающуюся в `end-1` и шагом `step`
...
# С помощью данной конструкции можно перевернуть список, имейте ввиду, что исходный список не
# меняется, а получается его копия
...
# * Даны два целых числа A и B (A &lt; B). Найти все целые числа, расположенные между данными
# числами (включая сами эти числа), в порядке их возрастания, а также количество N этих чисел.
...
# элементами исходного массива и его среднего арифметического.
# * Ввести массив, состоящий из 12 элементов действительного типа. Определить границы интервала,
# в котором находятся значения элементов массива.
# * Дан список целых чисел. Вернуть новый списов в котором все не отрицательные элементы
# возвведены в куб, в противном случае равны нулю
...
# **Логическое И**. Оператор ` and` возвращает `True` тогда и только тогда, когда оба его
...
# Записать условие принадлежности введеной координаты к координатной четверти через логические
# операторы и конструкцию if-elif-else
...
# * Записать логическое выражение, которые определяют, что число А не принадлежит интервалу
# от -10 до -1 или интервалу от 2 до 15.
# * Ввести с клавиатуры два числа. Определить, что больше, сумма квадратов или квадрат суммы
# этих чисел. Ответ вывести в виде сообщения.
# * Ввести с клавиатуры значения трех сторон треугольника a, b и c и определить, является ли он
# прямоугольным. Ответ вывести в виде сообщения.
# * Определить, является ли число a делителем для числа b и наоборот. Вывести сообщение
# * Даны значения трех сторон треугольника a, b, и c. Наименьшая из сторон треугольника является
# стороной квадрата. Определить, площадь какой из фигур больше
...
# * Написать логическое выражение для расчета суммы выплаты по депозиту на вклад : до 5000 грн
# начисляется 20% годовых, от 5000 грн до 10000 грн - 22% годовых.
...
# * Дано натуральное четырехзначное число. Выяснить, является ли оно палиндромом (читается
